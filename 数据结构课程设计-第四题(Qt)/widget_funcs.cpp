...
void Widget::divide_text() {
    ...
    for (int fileNum = 0; fileNum < pageNumber; fileNum++) {
        ...
        if (!f.open(QIODevice::Text | QIODevice::ReadOnly)) {
            QMessageBox::warning(
                this, tr("打开网页文本"),
                tr("打开网页文本文件%1错误：").arg(filename) + f.errorString());
        ...
        if (!indexfile.open(
                QIODevice::Text |
                QIODevice::WriteOnly))  //若要输出换行，需使用QIODevice::Text
        {
            QMessageBox::warning(
                this, tr("打开分词目标文件"),
                tr("打开分词目标文件%1错误：").arg(indexfilename) +
...
/* 将字符串根据其类型转为HTML语言 */
QString Widget::str_to_html(const QString& str, const vector<QString>& keywords,
                            int fileNum, int type) {
    QString html = QObject::tr("<p><pre>");
    if (type == TITLETYPE) {
        QString url =
            tr("http://%1").arg(QString::fromStdString(get_url(fileNum)));
        ...
        for (int i = 0; i < str.size(); i++) {
            ...
            if (flag >= 0)
                html += QObject::tr(
                            "<font family=\"Microsoft YaHei\" color=\"#ff2d51\" "
                            "size=\"5\">%1</font>")
                            .arg(str[i]);
            else
                html += QObject::tr(
                            "<font family=\"Microsoft YaHei\" color=\"#4b5cc4\" "
                            "size=\"5\">%1</font>")
                            .arg(str[i]);
        }
        html += QObject::tr("</a>");
        html += QObject::tr(
                    "<p><pre><font family=\"Microsoft YaHei\" color=\"#50616d\" "
                    "size=\"4\">PageRank=%1</font></pre></p>")
                    .arg(get_pr(fileNum));
        ...
        for (int i = 0; i < str.size(); i++) {
            ...
            if (flag >= 0)
                ...
                html += QObject::tr(
                            "<font family=\"Microsoft YaHei\" color=\"black\" "
                            "size=\"3\">%1</font>")
                            .arg(str[i]);
        }
    } else
...
void Widget::show_results(int* resultPages, int szResultPages,
    ...
    for (int i = 0; i < szResultPages; i++) {
        // QString fileName =
        // QObject::tr("%1/text/%2.txt").arg(entry).arg(resultPages[i]); QString
        // title = QObject::tr("http://%1").arg(
        // QString::fromStdString(get_url(resultPages[i])) ); qDebug() <<
        // "title=" << title;
