...
struct RID64_Dh {
    ...
    struct {
        ...
        uint8_t constMax_RID_idx;    // const 32: maximum (bit)size of RID, 2 as
                                     // the base
        uint8_t constMax_entry_idx;  // const 19: maximum (bit)size of .entry
                                     // section, 2 as the base
        uint8_t constMax_name_idx;   // const 19: maximum (bit)size of .name
                                     // section, 2 as the base
        uint8_t constMax_list_idx;   // const 31: maximum (bit)size of .list
                                     // section, 2 as the base
    };
};
...
struct RID64_Ih {
    struct {
        ...
        uint32_t constSize_node;   // const 2^5bytes: size of node, however in
                                   // expanded version, that could be 2^8bytes
        uint32_t constSize_entry;  // const 2^3bytes: size of entry, and that is
                                   // why all the struct in this standard is
                                   // prefixed with RID64_
    };
    ...
    uint8_t ABAND;  // decide whether to ignore num_node_H, name_off_H or
                    // init_off_H, 1 to ignore
    struct {
        uint16_t r1;
        uint16_t r2;
        uint16_t r3;
...
struct RID64_En {
    struct {
        ...
        uint16_t Fort : 1;   // used for quick check, if entry exactly matches
                             // the "cave"
        uint16_t
            tag : 13;  // used for quick check, storing tag of stored entry,
                       // which may not be coherent to whose sequence number(if
                       // not Fort, then not equal at any time), in some cases
                       // it becomes the middle 13 bits of init offset
        uint16_t
            Type : 1;  // define the type of the list it points to,
                       // 0 denotes that the list start at init, bearing
                       // {num_node_H:num_node} entries, 1 denotes that the list
                       // start at init and stop at particular node, 0xFFFF,
                       // while num_node and num_node_H is meaningless
    };
    ...
    struct {
        uint8_t
            num_node_H : 2;  // higher 2 bits of the number of nodes， whether
                             // abandoned or not decided by .index section
        uint8_t name_off_H : 3;  // higher 3 bits of name offset, whether
                                 // abandoned or not decided by .index section
        uint8_t init_off_H : 3;  // higher 3 bits of list head offset,  whether
                                 // abandoned or not decided by .index section
    };
};
...
// RID unit definition
// classes in this section manage the load, store, modify and initiate of the
// different sections the section includes: database heading, entry section,
// list section namesection and index heading the database heading tells the
// basic infomation of the RID file
//
class section_entry {
    uint32_t maxSize;
    uint32_t numCurrent;
...
int RI_itsc_symdif(std::string *request, int szRequest, int *&intersection,
                   int &szIntersection, int *&symDifference,
                   int &szSymdifference);
int RI_exclude(std::string base, std::string *exclude, int szExclude,
               int *&result, int &szResult);
void RID_terminate();
