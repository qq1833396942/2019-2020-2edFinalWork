void Widget::on_pushButtonSearch_clicked() {
    ...
    for (vector<string>::iterator iter = targetWords.begin();
         iter != targetWords.end(); iter++) {
        if (*iter == " ") targetWords.erase(iter);
    }

    /* 将vector类型转为动态数组供倒排索引使用 */
    int szRequest = targetWords.size();
    string *request = new string[szRequest];
    for (int i = 0; i < szRequest; i++) request[i] = targetWords[i];
    ...
    switch (searchType) {
        case AND:
            if (szIntersection <= 0) {
                ...
                QMessageBox::information(this, tr("搜索完成"),
                                         tr("搜索完成，未找到相关结果"));
                break;
            }
