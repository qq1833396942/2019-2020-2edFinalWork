...
InitRidThread::InitRidThread(QObject *parent) : QThread(parent) {}

void InitRidThread::run() {
    INDEX_DIR_PATH = string(
        (const char *)QObject::tr("%1/text-index/").arg(entry).toLocal8Bit());
    filenameRID = string(
        (const char *)QObject::tr("%1/RID.dat").arg(entry).toLocal8Bit());
