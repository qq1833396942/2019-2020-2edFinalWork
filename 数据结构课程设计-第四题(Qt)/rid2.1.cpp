 ...
 * in terms of complex ops on the tree, the maxWeb_num is set to a const to
 * avoid NO RESPONSE this modification will smooth the procedure, which is a
 * huge leap from the last version, which is troubled by uneven distribution of
 * keywords it is most costly procedure, please wait with patience
 */
void infoCollector(int webNum) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            targetNode_ptr->next = nullptr;
            map<string, infoCollector_node *>::iterator iter;
            iter = infoCollector_web.find(targetName);
            if (iter == infoCollector_web.end()) {
                infoCollector_web.insert(pair<string, infoCollector_node *>(
                    targetName, targetNode_ptr));
            } else {
                infoCollector_node *targetNode_scptr = iter->second;
                for (;
                     targetNode_scptr->freq > targetNode_ptr->freq &&
                     targetNode_scptr != nullptr && targetNode_scptr->next != nullptr;
                     targetNode_scptr = targetNode_scptr->next)
                    ;
                if (targetNode_scptr == nullptr) targetNode_scptr = iter->second;
...
void section_entry::write(uint32_t name_offset, pair<int, int> listInfo,
                          uint16_t afterhash, fstream &fn) {
    ...
    if (entrybuf.Valid == 0) {
        ...
        while (true) {
            ...
            while (_afterhash < 0) _afterhash += maxEntry;
            fn.seekg(startpos);
...
uint32_t section_entry::isAbandon() const { return H_abandon; }
streampos section_entry::position() const { return startpos; }
uint32_t section_entry::numEntry() const { return numCurrent; }
void section_entry::disArm() { security = false; }
/*list part*/
...
streampos list_section::positionCurrent() const { return listSection_pos; }
/*name part*/
uint32_t name_section::write(string request) {
    if (ptrCurrent + request.length() >= maxSize) return maxSize;
...
string name_section::read(uint32_t ask_offset) {
    if (ask_offset >= maxSize) return "";
    if (ask_offset > 0 && section_buf[ask_offset - 1] != '\0') {
        for (; ask_offset > 0 && section_buf[ask_offset] != '\0'; ask_offset--)
            ;
        if (ask_offset > 0) ask_offset++;
    }
    string result = section_buf + ask_offset;
    return result;
}
uint32_t name_section::end() const { return maxSize; }
uint32_t name_section::num() const { return numCurrent; }
uint32_t name_section::store(fstream &fn) {
    if (!fn) return 0;
    if (ptrCurrent == 0) return 0;
 ...
 * HASH_MAPING provides hash function to act hash mapping, in this scenario we
 * apply direct folding writeRID write file according to web
 * RID_initiator_internal is a slight collection of initiation
 * RID_loader load file to memory, which is a legacy of last version and in
 * order to quicken the process RID_initiation provide alternative initiation
 * ways, which is a legacy as well findKey is specialized function in order find
 * keywords in the files mark mark the existence of target string, which is
 * prepared for further logic algebra RI_itsc_symdif shows the intersection and
 * symmetry differences of requests RID_terminate release unused memory space,
 * which is a legacy as well
 */
#define nameBase_location(afterhash) \
    (((entryBase[afterhash].name_off_H << 16) + entryBase[afterhash].name_off))
#define listBase_location(afterhash) \
    (((entryBase[afterhash].init_off_H << 16) + entryBase[afterhash].init_off))
#define nodeNum(afterhash) \
    (((entryBase[afterhash].num_node_H << 8) + entryBase[afterhash].num_node))
...
bool InitRidThread::writeRID(fstream &fn) {
    ...
    databaseHead.LastMod_Time = time(nullptr);
...
void InitRidThread::RID_initiator_internal(int webNum) {
    ...
    while (!isTerminate) {
        ...
        _fn << "If you see these words, it indicates the failure of creating a "
               "RID file";
        _fn.close();
        fstream fn;
        assert(fn);
...
void InitRidThread::RID_initiation(int webNum) {
    ...
    WRITE:
        ...
        if (strcmp(bufDh.declaration, "RID")) goto WRITE;
...
int mark(string token, int *request) {
    ...
    for (; currentPage_idx <= currentPage_num; currentPage_idx++) {
        ...
        if (locationFirst != -1) {
            for (int firstcount = 0; firstcount < nodeNum(locationFirst);
                 firstcount++) {
                RID_Nd *node = (RID_Nd *)((char *)(void *)listBase +
                                          listBase_location(locationFirst)) +
                               firstcount;
...
// NOTICE: this function tells the intersection and symmetry difference of all
// the request strings the return value is the size of the union of the request
// strings request ptr is allocated from the outside, while intersection ptr and
// symDifference ptr otherwise
int RI_itsc_symdif(string *request, int szRequest, int *&intersection,
                   int &szIntersection, int *&symDifference,
                   int &szSymdifference) {
    if (!maxWeb_num) return 0;
    ...
    for (int requestsc = 0; requestsc < szRequest; requestsc++) {
        ...
        if (chk) {
            ...
            for (int i = 0; i < maxWeb_num; i++) _intsc[i] = 0;
        }
    }
...
// NOTICE: this function tells the set of webs which contains base but not
// exclude strings the math representation is:
//				x belongs to {webs | webs contains base, excluding
//those contain all the strings of exclude}
// the return value is the size of the result set
// result ptr is allocated from the inside, exclude ptr otherwise
int RI_exclude(string base, string *exclude, int szExclude, int *&result,
               int &szResult) {
    if (!maxWeb_num) return 0;
    int *buf = new int[maxWeb_num];
